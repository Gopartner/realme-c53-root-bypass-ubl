...
title RESTORE Realme C53 (RMX3760) - Stock Boot + Original VBMeta
...
if errorlevel 1 goto :ERROR
call :CHECK_FILE "%VBMETA_IMG%" "vbmeta_a_original.img"
if errorlevel 1 goto :ERROR
...
echo.
echo   %Y%Ini mengembalikan perangkat ke kondisi STOCK (tanpa root).%Z%
echo.
...
call :STEP 5 5 "Selesai"
echo.
echo   %G%%B%  RESTORE BERHASIL%Z%
echo.
echo   Perangkat sudah reboot ke fastboot.
echo   Langkah selanjutnya:
echo     %DIM%1. Boot ke Android (pilih reboot di fastboot)%Z%
echo     %DIM%2. Pastikan perangkat kembali normal (tanpa root)%Z%
echo.
pause
exit /b 0
...
echo   %C%%B%  STOCK BOOT + ORIGINAL VBMETA - VIA BROM%Z%
echo   %C%%B%====================================================%Z%
echo.
echo   Tujuan : kembalikan ke kondisi stock (tanpa root)
echo   Slot   : A
echo.
exit /b 0
